...
int main(int argc, char **argv) {
    ...
    if(rank % 2){
        randomize_cars_position(5, SIZE_X, SIZE_Y, ca_matrix, &vehicles, &vehicle_matrix);
        MPI_Send(&vehicles.front(), 5, com.elements_datatype, 0, 101, MPI_COMM_WORLD);
        std::cout << "sent." << std::endl;
    } else {
        vehicles.clear();
        vehicles.resize(5);
        MPI_Recv(&vehicles.front(), 5, com.elements_datatype, 1, 101, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
