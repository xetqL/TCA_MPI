...
std::pair<std::pair<int, int>, std::pair<int, int>> get_rotary_neighbors(int msx, int msy, int x, int y,
    ...
    xy = position_to_cell(msx, msy, x, y - 1 == -1 ? msy : y - 1); auto north_cell = ca_matrix.at(xy);
    xy = position_to_cell(msx, msy, x, y + 1 == msy ? -1 : y + 1); auto south_cell = ca_matrix.at(xy);
    xy = position_to_cell(msx, msy, x - 1 == -1 ? msx : x - 1, y); auto west_cell  = ca_matrix.at(xy);
    xy = position_to_cell(msx, msy, x + 1 == msx ? -1 : x + 1, y); auto east_cell  = ca_matrix.at(xy);
